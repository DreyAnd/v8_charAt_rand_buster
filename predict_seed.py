        ...
        for perm in itertools.combinations(permutations, l):
            ...
            for p in perm:
                ...
                if perm_samples_ is None or perm_multiple_ is None:
                    print("Impossible permutation:", p.__name__.split("perm_")[1])
                    break
...
def explain_permutations(perm, states, multiple):
    ...
    if perm_half_first in perm:
        print("- Only the first half of samples was used due to 64-wide cache boundary.")
    if perm_half_last in perm:
        print("- Only the last half of samples was used due to 64-wide cache boundary.")
    if perm_offset in perm:
        offset = get_offset(multiple)
        multiple -= offset
        print(f"- The samples were offset by {offset} to make them n-digit numbers.")
    if perm_minus_one in perm:
        print(f"- The multiple was decremented by one. Instead of {multiple}, it is {multiple-1}.")
        multiple -= 1

    print("Use the following command to predict new numbers:")
    print(f"./xs128p.py -s {states[0]},{states[1]} -m {multiple}{f' -a {offset}' if perm_offset in perm else ''} -g 5")


def convert_samples_to_int(samples):
    CHARSET = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'
    int_samples = []
    for line_no, sample in enumerate(samples, start=1):
        s = sample.strip()
        if not s:
            continue
        for ch in s:
            try:
                ...
                raise ValueError(f"Unknown character '{ch}' on line {line_no}")
            int_samples.append(idx)
    return int_samples
...
if __name__ == "__main__":
    ...
    parser.add_argument('--no-permute', action='store_true',
                        help='Disable trying permutations')

    args = parser.parse_args()

    if args.samples is not None:
        with args.samples.open() as f:
            args.samples = [line.strip() for line in f.readlines()]
    else:
        if os.isatty(sys.stdin.fileno()):
            print("Reading samples from stdin. Press Ctrl-D when done.")
        else:
            print("Reading samples from pipe.")
        args.samples = [line.strip() for line in sys.stdin.readlines()]

    print("Samples:", args.samples)
    assert len(args.samples) > 0, "Failed reading samples"

    args.samples = convert_samples_to_int(args.samples)
    if args.max_samples:
        args.samples = args.samples[:args.max_samples]
    print("Adapted samples:", args.samples)
    ...
    if args.no_permute:
        s, (state0_, state1_) = create_solver(args.samples, args.multiple)
        if s.check() == sat:
            m = s.model()
            state0 = m[state0_].as_long()
            state1 = m[state1_].as_long()
